...
local function parse_battery_info(content)
	local battery_info = {}
	for status, power, energy, charge in
		content:gmatch(".*STATUS=(%a+).+POWER_NOW=(%d+).+ENERGY_NOW=(%d+).+CAPACITY=(%d+)")
	do
...
function battery_widget.init(args)
	...
	monitor(unpack(merge(get_batteries(), {
		function(content)
			local charge, status, time = parse_battery_info(content)
			battery_widget.text:set_markup(
				beautiful.widget_markup:format(beautiful.bg_normal, math.floor(charge) .. "%")
			)

			if charge >= 0 and charge < 15 then
				batteryType = "battery-empty%s-symbolic"
				if
					not args.disable_battery_warning
					and status ~= "Charging"
