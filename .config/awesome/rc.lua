...
modalawesome.active_mode:connect_signal("widget::redraw_needed", function()
	...
	elseif text == "layout" then
		color = beautiful.lightgreen
	elseif text == "client" then
		color = beautiful.lightblue
	elseif text == "launcher" then
		color = beautiful.lightyellow
	end
...
local function clientmenu(filter, selected_tags_only)
	...
	if selected_tags_only then
		clients = gears.table.join(unpack(gears.table.map(function(t)
			return t:clients()
		end, scr.selected_tags)))
	else
		clients = client.get()
	end

	for c in
		gears.table.iterate(clients, function(c)
			return awful.rules.match(c, filter)
		end)
	do
		table.insert(items, {
			c.name,
...
modes.tag = gears.table.join({
	...
	},
	{
		...
		pattern = { "s", "t" },
		handler = function()
			clientmenu({ focusable = true }, true)
		end,
	},
	{
					...
					awful.spawn.easy_async_with_shell(cmd, function(stdout, stderr, _, code)
						if code ~= 0 then
							naughty.notify({
								...
								text = stderr ~= "" and stderr:gsub("%s$", "")
									or "Command terminated with exit code " .. code .. "!",
							})
						elseif stdout ~= "" then
			...
			run_shell.launch({
				...
				exe_callback = function(command)
					...
					awful.spawn.easy_async(browser .. search, function()
						local find_browser = function(c)
							return awful.rules.match(c, { class = browser, urgent = true })
						end
...
awful.rules.rules = {
	...
	{ rule = { class = "firefox" }, properties = { keys = awful.key({ "Control" }, "q", function() end) } },

	-- Make dragon sticky for easy drag and drop in ranger
	{ rule = { class = "Dragon-drop" }, properties = { ontop = true, sticky = true } },

	-- askpass has wrong height on multi-screen setups somehow
	{ rule = { class = "Git-gui--askpass" }, properties = { height = 200 } },
	...
	{ rule = { class = "mpv" }, properties = { size_hints_honor = true } },
